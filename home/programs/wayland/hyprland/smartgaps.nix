{
  ...
  in
    pkgs.writeShellScript "toggleSmartGaps" ''
      ...
      ${forEach (selector: ''
        hyprctl keyword workspace "${selector}, gapsout:10, gapsin:5"
        hyprctl keyword windowrulev2 "bordersize 2, floating:0, onworkspace:${selector}"
        hyprctl keyword windowrulev2 "rounding 16, floating:0, onworkspace:${selector}"
      '')}
      fi
    '';
in {
